...
- [Featured Flow Example](#featured-flow-example)
...
| [RNA-seq Flow Family](https://taffish.github.io/rnaseq-flows/) | First complete public flow-family portal built from TAFFISH executable packages, with manuals and a live yeast SNF2 example report. |
...
## Featured Flow Example

The [TAFFISH RNA-seq Flow Family](https://taffish.github.io/rnaseq-flows/) is
the first complete public flow-family example built from TAFFISH executable
packages. It shows how versioned command packages can compose into a
shell-native, auditable analysis route without turning TAFFISH into a workflow
engine.

The portal includes:

- a flow-family map for reference building, expression quantification,
  differential expression, enrichment, optional alignment/count/QC evidence,
  and reporting;
- per-flow manuals with input tables, parameters, outputs, and connection notes;
- a [live yeast SNF2 report](https://taffish.github.io/rnaseq-flows/examples/yeast-standard-report/04_reports/rnaseq_report.html)
  generated by the standard RNA-seq route.
